...
class DumpCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$input->hasOption('format') || in_array($format, array('json'))) {
            $formatter = $this->getContainer()->get('nayzo_api_doc.formatter.simple_formatter');
        } else {
            if (!in_array($format, $this->availableFormats)) {
                throw new \RuntimeException(sprintf('Format "%s" not supported.', $format));
            }

            $formatter = $this->getContainer()->get(sprintf('nayzo_api_doc.formatter.%s_formatter', $format));
        }
        ...
        $extractedDoc = $this->getContainer()->get('nayzo_api_doc.extractor.api_doc_extractor')->all($view);
